class Usuario:
    ...
    def adicionar_amizade(self, outro_usuario):
        if outro_usuario == self:
            print("Você não pode adicionar a si mesmo.")
            return
        if outro_usuario in self.lista_amigos:
    ...
    def remover_amizade(self, outro_usuario):
...
def atualizar_usuario(email, novo_nome=None, novo_email=None):
    usuario = procurar_usuario(email)
    if usuario is None:
        print("Usuário não encontrado.")
        return
